...
- **Сброс — в шапке поповера**: иконка-крестик справа от названия колонки
  (`EditorHeader`) возвращает колонку к нетронутому состоянию — тому же
  `defaultFilterFor`, что пишут ✕ на чипе и «Сбросить всё». Правит только
  черновик: применяет его «Применить», отменяет «Отмена»; пока сбрасывать
  нечего, кнопка неактивна. Подсказка на наведении — `table:table.reset`.
